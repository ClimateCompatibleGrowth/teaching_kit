...
for filename in material/nismod/Lectures/*/Mini-lecture*/Lecture_[0-9]*.docx; do
    ...
    if test -f "$BIBFILE"; then
        ...
        pandoc --standalone --css $PAN --css $CSL --citeproc $MD_NAME --bibliography tmp.bib -o output/$BNAME.html
    else
        pandoc --standalone --css $PAN --css $CSL $MD_NAME -o output/$BNAME.html
    fi
done;
# Clean up
rm tmp.bib
